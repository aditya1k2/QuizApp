...
class BookMarkAdapter(
    private val questions: ArrayList<Bookmark> = arrayListOf(),
    val selectedOption: (position: Int, selectedOption: Int) -> Unit
) : RecyclerView.Adapter<BookMarkAdapter.QuestionViewHolder>() {

    class QuestionViewHolder(val binding: ItemQuestionBinding) :
        RecyclerView.ViewHolder(binding.root) {
        fun bind(question: Bookmark) {
...
//            } ?: binding.optionsGroup.clearCheck()
    ...
    override fun onBindViewHolder(holder: QuestionViewHolder, position: Int) {
        ...
        holder.binding.optionsGroup.setOnCheckedChangeListener { radioGroup, checkedId ->
            val selectedPosition =
                radioGroup.indexOfChild(holder.binding.root.findViewById(checkedId))
            if (checkedId != -1) {
    ...
    fun updateData(questions: List<Bookmark>) {
